...
function UsagePanel({ isCollapsed }: { isCollapsed: boolean }) {
  ...
  const tenant = user?.tenantId ? state.tenants.find((t) => t.id === user.tenantId) : null
  ...
  const messagesUsage = planLimits.messages === -1 ? 0 : (messagesCount / planLimits.messages) * 100
  const synapsesUsage = planLimits.synapses === -1 ? 0 : (synapsesCount / planLimits.synapses) * 100
...
export function Sidebar({
  ...
  return (
    <motion.div
      ...
      className={cn("flex flex-col h-full border-r bg-card overflow-hidden")}
    >
      {/* Header */}
          ...
          {topItems.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href}>
                <motion.div whileHover={{ x: isCollapsed ? 0 : 2 }} transition={{ duration: 0.15 }}>
                  <Button
