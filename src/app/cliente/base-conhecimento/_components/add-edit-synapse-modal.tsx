...
interface AddEditSynapseModalProps {
  open: boolean
...
export function AddEditSynapseModal({
  open,
  onOpenChange,
  synapse,
  onSave,
}: AddEditSynapseModalProps) {
  const isEditMode = !!synapse
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>{isEditMode ? "Editar Synapse" : "Nova Synapse"}</DialogTitle>
          <DialogDescription>
            {isEditMode
              ? "Atualize as informações da synapse."
              : "Crie uma nova synapse para sua base de conhecimento."}
          </DialogDescription>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <DialogFooter>
              ...
              <Button type="submit">{isEditMode ? "Salvar Alterações" : "Criar Synapse"}</Button>
            </DialogFooter>
          </form>
        </Form>
      </DialogContent>
