...
interface InativarEmpresaModalProps {
  open: boolean
...
export function InativarEmpresaModal({
  open,
  onOpenChange,
  empresaNome,
  onConfirm,
}: InativarEmpresaModalProps) {
  const [confirmText, setConfirmText] = useState("")
  const CONFIRMATION_TEXT = "confirmo inativar empresa"
    ...
    >
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Esta ação irá inativar a empresa <strong>{empresaNome}</strong>. Todos os usuários
            perderão acesso ao sistema.
          </DialogDescription>
        </DialogHeader>
