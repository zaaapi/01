...
export function UsuarioSheet({
  ...
  const { state } = useData()
  const [modulosAtivos, setModulosAtivos] = useState<FeatureModuleKey[]>(usuario?.modules || [])
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="Ex: João Silva" {...field} disabled={isViewMode} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="+55 11 98765-4321" {...field} disabled={isViewMode} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
                      ...
                      <div key={modulo.key} className="border rounded-lg p-4">
                        <div className="flex items-start justify-between">
                          <div className="flex items-center gap-3">
                            <div
                              className={`rounded-lg p-2 ${modulosAtivos.includes(modulo.key) ? "bg-primary/10" : "bg-muted"}`}
                            >
                              <Icon
                                className={`h-5 w-5 ${modulosAtivos.includes(modulo.key) ? "text-primary" : "text-muted-foreground"}`}
                              />
                            </div>
                            <div>
